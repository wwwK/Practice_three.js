...
d3.csv("data.csv", function(data){
    ...
    if (window.matchMedia("(min-width: 1000px)").matches) {
        ...
        var radius = 9;
        var centerScale = d3.scalePoint().padding(1).range([0, w]);     // 넓이만큼 배치
        ...
        if (window.matchMedia("(min-width: 1000px)").matches) {
            ...
            if (window.scrollY >= window.innerWidth * 4 - 10 && checkOnce4) {
                ...
                d3.selectAll('circle')
                    ...
                    .style('stroke', function(d, i){return 'transparent'});
                toggleRender = true;
                setTimeout(() => document.querySelector('#c').style.zIndex = 1, 300);
            ...
            for (let i = 0; i < imageItems.length; i++){
                ...
                if ((window.scrollY >= window.innerWidth*2 - 10 && checkOnce)) {
                    console.log('why so slow');
                    // simulation.restart();
                    groupBubbles();
                    setTimeout(() => {
                ...
                // console.log(window.scrollY <= window.innerWidth - 10 && checkScrollUp && !checkOnce)
                if ((window.scrollY <= window.innerWidth*2 - 10) && checkScrollUp && !checkOnce) {
                    groupBubbles();
                    setTimeout(() => {
                        d3.selectAll('circle')
                            .style('fill', function(d, i){ return 'white' })
                ...
                if (window.scrollY >= window.innerWidth * 4 - 10 && checkOnce2) {
                    console.log('second');
                    splitBubbles('Group');
                    checkOnce2 = false;
                }
        
                if ((window.scrollY <= window.innerWidth * 4 - 10) && checkScrollUp && !checkOnce2) {
                    console.log('second');
                    splitBubbles('Group');
                ...
                if (window.scrollY >= window.innerWidth * 6 - 10 && checkOnce3) {
                    console.log('third point');
                    // groupBubbles();
                    // d3.selectAll('circle')
                    //     .attr("r", function(d, i){ return 1; })
                ...
                if (window.scrollY <= window.innerWidth * 6 - 10 && checkScrollUp && !checkOnce3) {
                    setTimeout(() => {
                    simulation.force('x', d3.forceX().strength(2).x(w / 4));
                    simulation.force('y', d3.forceY().strength(2).y(h / 2));
                    }, 100);
                ...
                if (window.scrollY >= window.innerWidth * 8 - 10 && checkOnce4) {
                    simulation.force('x')
                    d3.selectAll('circle')
                        .style('fill', function(d, i){return 'transparent'})
                        .style('stroke', function(d, i){return 'transparent'});
                ...
                if (window.scrollY <= window.innerWidth * 8 - 10 && checkScrollUp && !checkOnce4) {
                    console.log('canvas is moving back');
                    toggleRender = false;
                    document.querySelector('#c').style.zIndex = -1;
                    document.querySelector('#c').style.opacity = 0;
